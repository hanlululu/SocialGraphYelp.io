...
# ## Final Project: Network and text analytics upon the hotels of Las Vegas, using the Yelp review service
# 
# ### _by Yingrui Li (s171353), Sebastian Sbirna (s190553) and Hanlu He ()_
# ---
...
# --- 
# <a id='4.'></a>
# ## 4. Keyword detection using TF-IDF (NLP)
...
# 
# ![Image of Hotels Network Graph](https://i.imgur.com/4Urv8Ki.png)
...
# pyo.iplot(fig_network_community, filename = 'fig_network_community')
# fig_network_community.show()


# _Screenshot image, since the Plotly interactive graph does not show without re-running the notebook:_
# 
# ![Image of Communities Network Graph](https://i.imgur.com/ecCQid5.png)
...
# What can be understood from the analysis of this visual text plot is that ...
...
# 
# Normally, LDA can be applied when we are trying to understand the latent topics in a large document collection without knowing what kind of topics there will be. The main idea of it is "treat each document as a certain proportion of the topic collection, and each topic as a collection of keywords, again in a certain proportion". And the brief introduction of LDA is shown as below:
# 
# * 

# Once you provide the algorithm with the number of topics, it will rearrange the topic distribution in the document and the keyword distribution within the topic to get a good combination of topic-keyword distribution.
# 
# However, after performing community detection using Louvain algorithm, the original dataset containing reviews has been split into 4 main parts. So it can be expected that LDA model would 
...
total_review_text = pd.DataFrame(list(business_reviews.items()), columns = ['business_id', 'review']).review.apply(tokenize_text)
...
# Compute LDA model (pass num_topics = 4 as we have 4 communities)
...
# Since we already have looked at the community wordclouds in a previous section, we can see that the results fall very well in line with our findings: community nr. 1 was the community which contained most top keywords as swear words or inappropriate words such as 'strip'. 
# 
...
# <a id='7.2.'></a>
# ### 7.2. Temporal sentiment-score map
...
# 
# 1. Seasonal temporal map, where we will check if sentiment scores change based on the year quarter in which they were written
# 
...
# <a id='7.2.1.'></a>
# #### 7.2.1. Seasonal temporal map

# Here, we will compute sentiment scores of businesses in each quarter of year, to see if there is any variation between seasons:
...
# * Compute sentiment scores of businesses in each part of the day, to see if there is temporal change over different day time:
...
# Based on the segmentation of time, now it is possible to compute the sentiment score for every hotel during each time-period:
...
# Here, the geomap is showing the overall sentiments across each hotel in Las Vegas. This map does not have any temporal domain separation, and is included as a baseline reference of how the Las Vegas landscape looks like, in terms of sentiment analysis.
...
# As we can see, most hotels have a slighly positive experience rating, with the ones in the center of the city being more inclined to have a better overall score, while the perifery of the city starts seeing hotels with a rather-neutral outlook.

# > Geomap of sentiment in different quarter

# Using the above map plot as reference, we will now show the geo
